...
import { terminalsApi, type Terminal, type TerminalFormData } from '@/services/terminals-api';

export function TerminalsScreen() {
  const { items, loading, error, saving, reload, create, update, remove } = useCatalogCrud(terminalsApi);
  ...
  const openEdit = (item: Terminal) => {
    setEditing(item);
    setFormVisible(true);
  };

  const handleSave = async (values: TerminalFormData) => {
    try {
      if (editing) {
  ...
  const confirmDelete = (item: Terminal) => {
    Alert.alert('Eliminar', `¿Eliminar la terminal "${item.terminalCode}"? Esta acción no se puede deshacer.`, [
        ...
        ListEmptyComponent={
          !loading ? (
            <View style={styles.empty}>
              ...
              <Text style={[styles.emptyText, { color: colors.textSecondary }]}>No hay terminales registradas</Text>
            </View>
          ) : null
        }
      />
