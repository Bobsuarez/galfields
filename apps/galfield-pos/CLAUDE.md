...
- `catalog_sync.rs` — `sync_categories`/`sync_payment_methods`: the other two steps of the same "Sincronizar catálogo" chain, run right after `sync_products` (see `useCatalogSync.ts`). **The two commands use deliberately different reconciliation strategies**, because only one of the two local tables has something pointing at it:
  ...
  - `sync_payment_methods` pulls `GET /api/payment-methods` and **upserts by name** (`payment_method.name` is `UNIQUE`), then flips `is_active = 0` (migration `005_payment_method_active`) for any local row the cloud stopped returning — it deliberately never deletes, because `sales.payment_method` **is** a real foreign key (`PRAGMA foreign_keys = ON`), and deleting a row a past sale points to would fail outright. Same "never delete, just deactivate" fix as `sync_products`'s stale sweep above.
...
- **Gotcha — CDN hotlink protection returns 403 for the desktop app's own origin**: `cdn-galfields.kinforgeworks.com` (behind Cloudflare) 403s image requests whose `Referer` isn't the main `galfields.kinforgeworks.com` domain — confirmed with `curl -e "http://localhost:1420/" <url>` → 403 vs `curl` with no referer → 200 (User-Agent/Sec-Fetch-* headers don't matter, only `Referer` does). Since the desktop app's origin is `http://localhost:1420` in `npm run tauri dev` and `tauri://localhost` in a production build, every image request got blocked from this app specifically (mobile wasn't affected — React Native's image loader doesn't send a `Referer`). **Fixed via `index.html`'s `<meta name="referrer" content="no-referrer">`**, not the per-`<img>` `referrerpolicy` attribute — the attribute alone didn't work, most likely because WebKitGTK 2.52 (Tauri's Linux webview) doesn't reliably honor it on `<img>`; the page-wide meta tag is the older, more broadly-supported mechanism and suppresses the `Referer` on every outgoing request from the page, images included. **The `<img>`-level `referrerpolicy` attributes were left in place too** (harmless, defense-in-depth) but the meta tag is what actually fixes it — don't remove the meta tag thinking the per-element attribute already covers it. This is a workaround, not a real fix: the actual bug is the CDN's hotlink-protection rule contradicting `backend/pos`'s own design (see that repo's CLAUDE.md's "Image URLs / CDN" section — these URLs are meant to be public/unsigned/directly embeddable by any client). The durable fix is disabling hotlink protection (or whitelisting an empty referrer) on the Cloudflare zone/CDN config, which is outside both this repo and `backend/pos`. **Note:** a page-wide meta tag only takes effect on a fresh page load — after pulling this change, fully restart `npm run tauri dev` (not just wait for Vite HMR) before testing.
