...
/** Append-only shadow of {@code stock_adjustments}, backing the
 * (clientEventId, variantId) idempotency check — see
 * InventoryShadowEntity's javadoc. {@code created_at} isn't mapped: DB-level
 * DEFAULT CURRENT_TIMESTAMP, no need to set it from Java. */
@Entity
@Table(name = "stock_adjustments", uniqueConstraints = @UniqueConstraint(columnNames = {"client_event_id", "variant_id"}))
@Getter
@Setter
public class StockAdjustmentShadowEntity {
